...
if [ $# -eq 0 ]
   then
    ...
    echo "-----------------------------------------------------"
    echo "initraid0      = Initialize the specified slots as RAID0 drives"
...
# Loop through the defined slots in arguments and create individual RAID0 volumes.
if [ $1 = "initraid0" ]
   then
      slot=$(($(lsblk | grep disk | wc -l)+1))
      lastslot=$(($(MegaCli -pdList -a0 | grep -i -E "slot" | wc -l)-1))
      deviceid=$(MegaCli -pdList -a0 | grep "Enclosure Device ID:" | head -n 1 | cut -d ':' -f 2 | cut -d ' ' -f 2)
      echo "###############################################"
      while [ $slot -le $lastslot ]
          do
              $MegaCli -cfgldadd -r0[${deviceid}:${slot}] WB RA Cached CachedBadBBU -strpsz512 -a0 -NoLog
              slot=`expr $slot + 1`
              echo "###############################################"
          done
fi
